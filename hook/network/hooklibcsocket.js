...
const so_name = "libmonosgen-2.0.so"
const so_addr = Module.findBaseAddress(so_name)
function printNativeStack(context, name) {
    ...
    if (first.toString().indexOf('libopenjdk.so!NET_Send') < 0) {
        var trace = Thread.backtrace(context, Backtracer.ACCURATE).map(function (value) {
            var symbol = DebugSymbol.fromAddress(value);
            // console.log(symbol.moduleName);
            if (symbol.moduleName === so_name) {
                return symbol + " offset: " + value.sub(so_addr);
            }
            return symbol;
        }).join('\n');
        LogPrint("-----------start:" + name + "--------------");
